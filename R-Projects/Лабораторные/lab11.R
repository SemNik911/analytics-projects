...
my_vector <- c(26.453 + 0.1*i, 26.971 + 0.1*i, 27.062 + 0.1*i, 27.543 + 0.1*i,
               28.536 + 0.1*i, 28.776 + 0.1*i, 28.915 + 0.1*i, 29.519 + 0.1*i,
               30.556 + 0.1*i, 30.722 + 0.1*i, 30.856 + 0.1*i, 31.612 + 0.1*i)
my_vector
smoothed_data <- c(26.453 + 0.1*i)
smoothed_data
alpha = 0.2
...
#Определить тренд для аддитивной модели временного ряда.
...
skorrektirovannaya_sezonnaya_komponenta
for (i in 1:length(my_vector)) {
  predict[i] = trendovoe_znachenie[1] + delta * (i + 8) + skorrektirovannaya_sezonnaya_komponenta[(i - 1) %% 4 + 1]
}
predict
